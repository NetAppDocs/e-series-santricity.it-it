...
---
= Modificare le impostazioni della cache per un volume in SANtricity System Manager
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
. Fare clic sull'icona del menu:ellipsis [Change volume cache settings].
+
Viene visualizzata la finestra "Modifica impostazioni cache volume". Tutti i volumi sull'array di storage vengono visualizzati in questa finestra di dialogo.

. Seleziona la casella accanto a ciascun volume per cui desideri modificare le impostazioni della cache oppure seleziona la casella nell'intestazione della tabella per selezionare tutti i volumi.
+
[cols="25h,~"]
|===
...
|===
. Fai clic su *Salva* per modificare le impostazioni della cache
