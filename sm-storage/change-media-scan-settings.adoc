...
. Fare clic sull'icona menu:ellipsis [Change volume media scan settings].
+
Viene visualizzata la finestra "Modifica impostazioni scansione supporti unità". Tutti i volumi sullo storage array vengono visualizzati in questa finestra di dialogo.

. Per abilitare la scansione dei supporti, selezionare la check box *Media scan*.
+
