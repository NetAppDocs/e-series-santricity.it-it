...
NOTE: La funzione Check Volume Redundancy è attualmente disponibile solo per le operazioni nell'interfaccia Legacy System Manager.

. Dal pannello di sinistra, selezionare menu:Storage[Pool e gruppi di volumi].
. Fai clic sull'icona menu:ellipsis [Check volume redundancy] per il pool o volume group che desideri controllare.
+
