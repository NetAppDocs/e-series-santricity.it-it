...
+
Aggiungi capacità a un volume selezionando menu:Storage[Volumes] dal pannello di sinistra e quindi selezionando il menu:ellipsis icon[Increase capacity] per il volume.
...
+
Aggiungi capacità non assegnata al pool o al gruppo di volumi selezionando menu:Storage[Pools and Volume Groups > Add capacity].
...
Quando si copiano i volumi, si crea una copia point-in-time di due volumi distinti, il volume di origine e il volume di destinazione, sullo stesso storage array. È possibile copiare i volumi selezionando menu:Storage[Volumes] e quindi selezionando il menu:ellipsis icon[Copy volume].
...
È possibile inizializzare i volumi selezionando menu:Storage[Volumes > ellipsis icon > Initialize volumes].
...
È possibile ridistribuire i volumi selezionando menu:Storage[Volumes > ellipsis icon > Redistribute volumes].



== Modificare la proprietà del volume

La modifica della proprietà di un volume modifica il controller proprietario preferito del volume. Il controller proprietario preferito di un volume è elencato nel menu:Storage[Volumes > ellipsis icon > Edit settings].

È possibile modificare la proprietà di un volume selezionando menu:Storage[Volumes > ellipsis icon > Change volume ownership].
...
Selezionare menu:Storage [Volumes > ellipsis icon > Change cache settings] per modificare le seguenti impostazioni della cache:
