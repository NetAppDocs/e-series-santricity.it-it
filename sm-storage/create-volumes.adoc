...
+
Viene visualizzata la pagina Create Volumes.
...
. Fare clic su *Avanti*.
. Se il tuo carico di lavoro è associato a un tipo di applicazione supportato, inserisci le informazioni richieste; altrimenti, vai a <<Passaggio 3: Aggiungi volumi>>.




== Passaggio 3: Aggiungi volumi
...
+
** 2.048 volumi (EF80, EF600, EF600C e serie E5700)
** 1.024 volumi (EF50, EF300 e EF300C)
...
DA aumenta l'integrità dei dati nell'intero sistema storage. DA consente allo storage array di controllare gli errori che potrebbero verificarsi quando i dati vengono trasferiti attraverso i controller fino ai dischi. L'utilizzo di da per il nuovo volume garantisce il rilevamento di eventuali errori.
 ...
 a| 
Risorsa allocata (EF80, EF50, EF300 e EF600 solo)
 a| 
*Sì* viene visualizzato accanto a "Resource provisioned" solo se le unità supportano questa opzione. Resource Provisioning è una funzionalità disponibile negli array di storage EF80, EF50, EF300 e EF600, che consente di mettere in uso i volumi immediatamente senza alcun processo di inizializzazione in background.

|===
====
** *Carico di lavoro specifico dell'applicazione* -- Fare clic su *Modifica volumi* per modificare, aggiungere o eliminare i volumi e le caratteristiche consigliati dal sistema per il workload selezionato.
+
.Dettagli del campo
[%collapsible]
====
...
Il tipo di volume indica il tipo di volume creato per un carico di lavoro specifico dell'applicazione.
 ...
 a| 
Dimensione del blocco volume (EF80, EF50, EF300 e EF600 solo)
 a| 
Mostra le dimensioni del blocco che è possibile creare per il volume:
