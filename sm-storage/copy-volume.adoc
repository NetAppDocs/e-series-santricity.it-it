...
. Seleziona il volume che desideri utilizzare come origine per l'operazione Copy Volume, quindi fai clic sull'icona menu:ellipsis icon[Copy volume].
+
Viene visualizzata la finestra Copy Volume.
...
.Al termine
Selezionare menu:Storage[Operations in Progress] per visualizzare l'avanzamento dell'operazione Copy Volume. Questa operazione può essere lunga e potrebbe influire sulle prestazioni del sistema.
