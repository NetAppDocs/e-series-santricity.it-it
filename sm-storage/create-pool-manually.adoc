...
---
= Creare manualmente un pool in SANtricity System Manager
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
DA è abilitato se tutte le unità sono compatibili con DA. DA può essere disabilitato dopo la creazione del volume selezionando menu:Storage[Volumes > ellipsis icon > Disable Data Assurance].



 a| 
Resource Provisioning Capable (solo EF80, EF50, EF300 e EF600)
 a| 
Indica se Resource Provisioning è disponibile per questo pool candidato. Resource Provisioning è una funzionalità disponibile negli array di storage EF80, EF50, EF300 e EF600, che consente di mettere in uso i volumi immediatamente senza alcun processo di inizializzazione in background.
