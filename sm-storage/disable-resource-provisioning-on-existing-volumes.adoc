...
Il provisioning delle risorse è una funzionalità disponibile negli array di storage EF80, EF50, EF300/EF300C e EF600/EF600C, che consente di mettere in uso i volumi immediatamente senza alcun processo di inizializzazione in background. Tutti i blocchi di unità assegnati al volume vengono deallocati (smappati), il che può migliorare la durata degli SSD e aumentare le prestazioni massime di scrittura.
...
. Fai clic sull'icona *ellipsis* e seleziona *Enable resource provisioning* oppure *Disable resource provisioning* per il pool o il gruppo di volumi selezionato.
