...
+
Per creare manualmente un pool: selezionare *Archiviazione*, quindi selezionare *Pool e gruppi di volumi*. Dalla scheda Tutta la capacità, fare clic su menu:Create[Pool].
...
+
Per aggiungere manualmente unità a un pool esistente: dalla pagina Pools and volume groups, selezionare il pool e quindi fare clic su *Add capacity*.
...
È possibile modificare la capacità di conservazione da *Pools and volume groups*. Selezionare il pool che si desidera modificare. Fare clic sull'icona del menu:ellipsis [Edit Settings] e quindi selezionare la scheda *Settings*.
...
Nella finestra di dialogo Aggiungi capacità, non tutte le unità sono disponibili per aggiungere capacità a un pool o a un volume group esistente.
...
** Il gruppo di volumi deve contenere un numero pari di unità. Se non hai un numero pari di unità e hai alcune unità non assegnate rimanenti, vai su *Pools and volume groups* per aggiungere altre unità al gruppo di volumi e riprova l'operazione.
...
Per visualizzare le statistiche della Cache SSD, seleziona menu:Storage[Pools and volume groups] dal pannello di sinistra. Seleziona la Cache SSD per cui desideri visualizzare le statistiche, quindi fai clic sull'icona menu:ellipsis icon[View statistics]. Le statistiche nominali vengono mostrate nella pagina View Statistics.
...
[NOTE]
====
Il cursore Additional Optimization Capacity è disponibile solo per i sistemi di storage EF80, EF50, EF600 e EF300.
...
Il Resource Provisioning è una funzionalità disponibile negli array di storage EF80, EF50, EF300 e EF600, che consente di mettere in uso i volumi immediatamente senza alcun processo di inizializzazione in background.
...
Il provisioning delle risorse è abilitato per impostazione predefinita sui sistemi in cui le unità supportano DULBE. È possibile disabilitare questa impostazione predefinita da *Pools and volume groups*.
