...
---
= Aggiungere capacità a un pool o a un gruppo di volumi in SANtricity System Manager
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
+
** 2,048 volumi (EF80, EF600 e E5700 series)
** 1,024 volumi (EF50 e EF300)
...
[NOTE]
====
Evitate di utilizzare unità compatibili con Data Assurance (DA) per aggiungere capacità a un pool o a un gruppo di volumi non compatibile con DA. Il pool o il gruppo di volumi non può sfruttare le funzionalità dell'unità compatibile con DA. Considerate l'utilizzo di unità non compatibili con DA in questa situazione.

====
.Fasi
. Dal pannello di sinistra, selezionare menu:Storage[Pool e gruppi di volumi].
. Seleziona il pool o il gruppo di volumi a cui desideri aggiungere unità, quindi fai clic sull'icona menu:ellipsis icon[Aggiungi capacità].
+
Viene visualizzata la finestra di dialogo Aggiungi capacità. Vengono visualizzate solo le unità non assegnate compatibili con il pool o il gruppo di volumi.

. In *Select drives to Add capacity...*, seleziona una o più unità che desideri aggiungere al pool o al gruppo di volumi esistente.
+
...
** Si sconsiglia di utilizzare unità non compatibili con Data Assurance (DA) per aggiungere capacità a un pool o a un gruppo di volumi compatibile con DA. Il pool o il gruppo di volumi non avrà più funzionalità DA e non sarà più possibile abilitare DA sui volumi appena creati all'interno del pool o del gruppo di volumi.
** L'utilizzo di unità che sono Data Assurance (DA) capable per aggiungere capacità a un pool o a un gruppo di volumi che non è DA-capable non è raccomandato, perché quel pool o gruppo di volumi non può sfruttare le capacità dell'unità DA-capable (gli attributi dell'unità non corrispondono). Considera di utilizzare unità che non sono DA-capable in questa situazione.
...
Compatibile con DULBE
 a| 
Indica se l'unità dispone dell'opzione per Deallocated or Unwritten Logical Block Error (DULBE). DULBE è un'opzione presente sulle unità NVMe che consente all'array di storage EF80, EF50, EF300 o EF600 di supportare volumi resource-provisioned.
