...
---
= Consolidare la capacità libera per un gruppo di volumi in SANtricity System Manager
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
* È inoltre possibile avviare la finestra di dialogo Consolidate Free Capacity dalla pagina Pools e gruppi di volumi, come descritto nell'attività seguente.
...
. Fare clic sull'icona menu:ellipsis [Consolidate volume group free capacity].
+
...
.Al termine
Selezionare menu:Storage[Operations in Progress] per visualizzare l'avanzamento dell'operazione Consolidate Free Capacity. Questa operazione può essere lunga e potrebbe influire sulle prestazioni del sistema.
